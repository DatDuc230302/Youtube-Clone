...
function UploadForm({ turn, setTurn }) {
    ...
    async function post() {
        try {
            if (type === 'type') {
                ...
                alert('Vui lòng chọn check');
            } else {
                if (!content || !img || !avatar || !name || !time || !link || !subscriber || !likes || !description) {
                    alert('Vui lòng điền đầy đủ thông tin');
                } else {
                    const checkBody: boolean = check === 'true' ? true : false;
                    const linkBody = link.slice(-11);
                    const body = {
                        content,
                        img,
                        avatar,
                    ...
                    const res = await axios.post('https://server-youtube.onrender.com/watches/post', body);
                    if (res.status === 200) {
                        alert('Bạn đã post dữ liệu thành công !');
                    }
                }
            }
        } catch {
            console.log('Post to server fail');
        }
    }
    return (
        <div className={cx('wrapper', turn && 'turn')}>
            <div onClick={() => setTurn(true)} className={cx('overlay')}></div>
                ...
                <div className={cx('form')}>
                    ...
                    <div className={cx('control')}>
                        ...
                        <input
                            ...
                            className={cx('control-input')}
                        />
                        <select className={cx('control-input')} onChange={(e) => setCheck(e.target.value)}>
                        ...
                        <select className={cx('control-input')} onChange={(e) => setType(e.target.value)}>
                            <option value={'music'}>choose type</option>
                        ...
                        <input
                            ...
                            className={cx('control-input')}
                        />
                        <button onClick={post} className={cx('control-btn')}>
                            Post
                        </button>
                        <button onClick={() => setTurn(true)} className={cx('control-btn')}>
                            Cancle
                        </button>
                    </div>
                </div>
